 ...
 *
 * Part of brackets are licenced under the GNU GPL. See README and COPYING for details.
 */
 ...
 * $url is the part appended to the base brackets $resturl.
...
function request($url, $verb = 'GET', $data = '', $failonerror = true) {
	...
	$ch = curl_init($url);
	curl_setopt($ch, CURLOPT_USERAGENT, "[brackeys]/v1.0.0");
...
logmsg(LOG_NOTICE, "Judge started on $myhost [brackets/v1.0.0]");
...
function judge($row)
{
	...
	// Make sure the workdir is accessible for the brackets-run user.
	...
	if( ! isset($EXITCODES[$retval]) ) {
		...
		// revoke readablity for brackets-run user to this workdir
		chmod($workdir, 0700);
		return;
	}
