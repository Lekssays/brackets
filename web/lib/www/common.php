...
function addInputField($type, $name = null, $value = null, $attributes = '') {
    ...
    if($type == 'reset'){
		return '<input class="ui red button" type="'.$type.'"'.
			($name  !== null ? ' name="'.specialchars($name).'"' : '') . $id .
			($value !== null ? ' value="'.specialchars($value).'"' : '') .
			' ' . $attributes . " />\n";    	
    } else if ($type == 'submit'){
		return '<input class="ui green button" type="'.$type.'"'.
			($name  !== null ? ' name="'.specialchars($name).'"' : '') . $id .
			($value !== null ? ' value="'.specialchars($value).'"' : '') .
			' ' . $attributes . " />\n";  
    } else {
		return '<input class="ui teal button" type="'.$type.'"'.
			($name  !== null ? ' name="'.specialchars($name).'"' : '') . $id .
			($value !== null ? ' value="'.specialchars($value).'"' : '') .
			' ' . $attributes . " />\n";      	
    }
...
function addForm($action, $method = 'post', $id = '', $enctype = '', $charset = '', $extra = '')
{
	...
	return '<form class="ui form" style="display:inline;" action="'. $action .'" method="'. $method .'"'.
